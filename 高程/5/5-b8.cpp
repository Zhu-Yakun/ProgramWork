...
void calc_num(char pure_decimal[], char out[], char power_table[][N_NUM + 1])
{
	...
	for (int i = 1; i < N_POW; i++)
	{
		if (pure_decimal[i])
		{
			for (int k = N_POW - 1; k > 0; k--)
			{
				tmp = out[k] + power_table[i][k];
				out[k] = tmp % 10;
				if (tmp > 9)
					out[k - 1] += tmp / 10;
			}
...
/***************************************************************************
  ...
  说    明：
 ***************************************************************************/
void input_pure_decimal(char pure_decimal[], int flag)
{
	char ch;
	int i = 0;
	bool prime = 1, flag1 = 0, flag2 = 1;
	if(flag)
	...
	else if (!flag1)
	{
		...
		for (int j = 0; j < i; j++)
			pure_decimal[j] = 0;
		input_pure_decimal(pure_decimal, 0);
	}
	else if (!flag2)
	{
...
int main()
{
	...
	*/
	...
	input_pure_decimal(pure_decimal, 1);
