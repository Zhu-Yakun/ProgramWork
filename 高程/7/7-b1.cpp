...
struct tj_time* tj_time_convert(int input_time)
{
	...
	input_time += 8 * 60 * 60;
	int num_day = input_time / (24 * 60 * 60);
	int N = num_day;
	result.tj_hour = (input_time - num_day * 24 * 60 * 60) / (60 * 60);
	input_time -= num_day * 24 * 60 * 60;
	result.tj_minute = (input_time - result.tj_hour * 60 * 60) / 60;
	result.tj_second = (input_time - result.tj_hour * 60 * 60) - result.tj_minute * 60;
	int num_year = 0, day_of_year = 365, m_day, m = 1;
	bool is_leap = 0;
	while (num_day >= 0) {
		if (num_day >= (day_of_year + ((num_year - 2) % 4 == 0))) {
			num_day -= (day_of_year + ((num_year - 2) % 4 == 0));
			num_year++;
		}
		else {
			result.tj_year = num_year + 1970;
			if (num_year > 1)
				is_leap = ((num_year - 2) % 4 == 0);
			break;
		}
	}
	while (num_day >= 0) {
